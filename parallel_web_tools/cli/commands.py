...
def _build_command_schema(cmd: click.Command, prefix: str = "parallel-cli") -> dict[str, Any]:
    """Build a JSON schema for a Click command or group."""
    name = prefix if cmd.name in ("main", None) else f"{prefix} {cmd.name}"
    info: dict[str, Any] = {"name": name}

    if cmd.help:
        # Take first line of help text, strip Click \b markers
        info["description"] = cmd.help.replace("\b", "").strip().split("\n")[0]

    if isinstance(cmd, click.Group):
        info["type"] = "group"
        info["commands"] = [
            _build_command_schema(sub_cmd, prefix=name) for sub_name, sub_cmd in sorted(cmd.commands.items())
        ]
    else:
        info["type"] = "command"
        arguments = []
        ...
        for param in cmd.params:
            param_info: dict[str, Any] = {"name": param.name or ""}
            if isinstance(param, click.Argument):
                ...
                elif param.type and param.type.name != "TEXT":
                    param_info["type"] = param.type.name
                if param.default is not None and not param.is_flag:
                    default = param.default
                    # Filter out Click sentinel values
                    if not callable(default) and "Sentinel" not in type(default).__name__:
...
def schema_cmd():
    """Output the CLI command structure as JSON for programmatic discovery."""
    schema = _build_command_schema(main)
    schema["version"] = __version__
