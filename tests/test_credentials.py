...
class TestLoad:
    ...
    def test_load_prefers_existing_auth_file_over_legacy_file(self, creds_file, legacy_file):
        creds_file.parent.mkdir(parents=True, exist_ok=True)
        creds_file.write_text(
            json.dumps({"version": 1, "selected_org_id": None, "orgs": {}, "client_id": "cid_registered"})
        )
        legacy_file.write_text(json.dumps({"access_token": "tok_v0"}))
        ...
        assert loaded.client_id == "cid_registered"
        assert loaded.selected_org_id is None
        assert loaded.orgs == {}

    def test_migrated_v0_has_no_client_id(self, creds_file, legacy_file):
        ...
        assert loaded.orgs[LEGACY_ORG_ID].org_name is None


class TestSave:
    def test_save_creates_parent_dir(self, tmp_path, monkeypatch):
        path = tmp_path / "nested" / "subdir" / "auth.json"
        monkeypatch.setattr(credentials, "CREDENTIALS_FILE", path)
        monkeypatch.setattr(credentials, "AUTH_FILE", path)
        monkeypatch.setattr(credentials, "LEGACY_CREDENTIALS_FILE", tmp_path / "nested" / "subdir" / "credentials.json")
        save(Credentials())
        assert path.exists()
    ...
    def test_save_does_not_write_legacy_file(self, creds_file, legacy_file):
        save(Credentials(selected_org_id="x", orgs={"x": OrgCredentials(api_key="k")}))
        assert not legacy_file.exists()
    ...
    def test_atomic_write_preserves_existing_on_failure(self, creds_file, monkeypatch):
        ...
        leftovers = [p for p in creds_file.parent.iterdir() if p.name.startswith(".auth.")]
        assert leftovers == []
...
class TestDelete:
    ...
    def test_delete_removes_legacy_file_too(self, creds_file, legacy_file):
        save(Credentials(selected_org_id="x", orgs={"x": OrgCredentials(api_key="k")}))
        assert delete() is True
        assert not creds_file.exists()
        assert not legacy_file.exists()
